...
export class DiffParser {
  ...
  private parseHunk(
    ...
    return new DiffHunk(
      ...
      this.getHunkHeaderExpansionType(hunkIndex, header, previousHunk)
    )
  }

  /**
   * Determine the expansion type for a hunk based on its position
   */
  private getHunkHeaderExpansionType(
    hunkIndex: number,
    header: DiffHunkHeader,
    previousHunk: DiffHunk | null
  ): HunkExpansionType {
    if (hunkIndex === 0) {
      return previousHunk === null ? HunkExpansionType.Single : HunkExpansionType.First
    }
    
    return HunkExpansionType.Middle
  }

  /**
   * Get the largest line number from all hunks
   */
  private getLargestLineNumber(hunks: readonly DiffHunk[]): number {
    let max = 0
  ...
  public parse(text: string): IRawDiff {
    ...
    try {
      ...
      return {
        ...
        maxLineNumber: this.getLargestLineNumber(hunks),
