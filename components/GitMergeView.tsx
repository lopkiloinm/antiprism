...
import { IconAlignLeft, IconLayoutGrid } from "./Icons";
...
const gitMergeViewStyles = `
  .git-merge-root,
  .git-merge-root .cm-mergeView {
    height: 100%;
    min-height: 0;
  ...
  .git-merge-root .cm-mergeViewEditors {
    display: flex;
    align-items: stretch;
    min-height: 100%;
  }

  .git-merge-root .cm-mergeViewEditor {
    flex: 1 1 0;
    min-width: 0;
    overflow: auto;
  }

  .git-merge-root .cm-editor {
    position: relative !important;
    height: auto !important;
    min-height: 100%;
  }

  .git-merge-root .cm-mergeViewEditor + .cm-mergeViewEditor {
    border-left: 1px solid var(--border);
  }

  .git-merge-root .cm-scroller {
    position: relative !important;
    top: auto !important;
  ...
  .git-merge-root .cm-gutters {
    background: var(--cm-gutter-bg, color-mix(in srgb, var(--background) 92%, black));
  }

  .git-merge-root .cm-merge-a .cm-changedLine,
  .git-merge-root .cm-deletedChunk {
    background: rgba(255, 68, 68, 0.12);
  }

  .git-merge-root .cm-merge-b .cm-changedLine,
  .git-merge-root .cm-inlineChangedLine {
...
function getBaseTheme(theme: Theme, fontSize: number) {
  const isLightTheme = theme === "light";
...
export function GitMergeView({
  ...
  useEffect(() => {
    const styleId = "git-merge-view-styles";
    if (!document.getElementById(styleId)) {
      const style = document.createElement("style");
      style.id = styleId;
      style.textContent = gitMergeViewStyles;
  ...
  useEffect(() => {
    ...
    const extensions: Extension[] = [
      ...
      drawSelection(),
      highlightActiveLine(),
      syntaxHighlighting(getHighlight(theme)),
      ...getBaseTheme(theme, fontSize),
  ...
  }, [currentContent, filePath, fontSize, lineWrapping, originalContent, theme, typstSupport, viewMode]);

  return <div ref={containerRef} className={`git-merge-root h-full min-h-0 overflow-auto ${className}`.trim()} />;
}
